...
def _render_operation_photos() -> None:
    ...
    for sub in submissions:
        ...
        # ── Photo expander (full-width, below row) ────────────────────────────
        if n_photos:
            ...
            for ci in all_ci:
                ...
                with st.container(border=True):
                    ...
                    if _btn1.button("💾 Save", key=f"savebtn_{cid}", width='stretch'):
                        ...
                        if _stamped_bytes:
                            # Write stamped bytes back to disk + Supabase so the
                            # stamp carries through to Send-to-Accounting and the
                            # accounting dashboard (generate_pdf reads this file).
